...
mod4.js adds and controls functionality of wizard and policy
  (Section 4 of the wizard and policy)
*/


import { practices } from './../wizard.js'
import { updateAppName} from './../utilities.js'
import { syncMod6 } from './mod6.js'
...
function updatePolicy(){
  ...
  if ($("#mod-4w-r0-yes").prop("checked") && 
    $("#mod-4w-r1-yes").prop("checked")) {
    $('#mod-4p-content').html(html1);
    syncMod6(true)
  } else if ($("#mod-4w-r0-yes").prop("checked")) {
    $('#mod-4p-content').html(html2);
    syncMod6(true)
  } else if ($("#mod-4w-r1-yes").prop("checked")) {
    $('#mod-4p-content').html(html3);
    syncMod6(true)
  } else {
    $('#mod-4p-content').html(html4);
    syncMod6(false)
  }
  updateAppName();
}
...
function updateWizard(){
  ...
  $('#mod-4w-r1-yes').click(function(){
    updatePolicy()
  })
  $('#mod-4w-r1-no').click(function(){
    updatePolicy()
  })
  ...
  if (practices["GOOGLE"]["used"] == true) {
    $("#mod-4w-r1-yes").prop("checked", true);
  } else {
    $("#mod-4w-r1-no").prop("checked", true);
  }
