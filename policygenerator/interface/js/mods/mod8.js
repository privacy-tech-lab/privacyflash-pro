...
function setDefaults(){
  $('#mod-8w-r0-yes, #mod-8w-r1-yes, #mod-8w-r2-yes, #mod-8w-r0-no, #mod-8w-r1-no, #mod-8w-r2-no')
  .click(function(){
    if (!$("input:radio[name='mod-8w-r0']").is(":checked") || 
      !$("input:radio[name='mod-8w-r1']").is(":checked") ||
      !$("input:radio[name='mod-8w-r2']").is(":checked")
    ) {
      updatePolicy()
      $('#mod-8w-collapse').collapse('hide')
    } else if ($('#mod-8w-r2-yes').prop("checked") && 
    ($('#mod-8w-r1-yes').prop("checked") || $('#mod-8w-r0-yes').prop("checked"))) {
      $('#mod-8w-coppa-info').html('COPPA is applicable to your app.')
      $('#mod-8w-more, #mod-8w-coppa-info').fadeIn()
      updatePolicy()
    } else {
      $('#mod-8w-coppa-info').fadeIn()
      $('#mod-8w-coppa-info').html('COPPA is not applicable to your app.')
      $('#mod-8w-more').fadeOut()
      updatePolicy()
      $('#mod-8w-collapse').collapse('hide')
    }
  })
  $('#mod-8w-more').click(function(){
    $('#mod-8w-collapse').collapse('show')
  })
  ...
  $('#mod-8w-r3-yes').click(function (){
    $('.mod-8w-input-0').fadeIn()
    updatePolicy()
  })
  $('#mod-8w-r3-no').click(function (){
    $('.mod-8w-input-0').fadeOut()
    updatePolicy()
  })
  $('#mod-8w-input-0').on('input', function(){
    updatePolicy()
  })
  $('#mod-8w-input-1').on('input', function(){
    updatePolicy()
  })

  inflateFPInfoBtns()
  $('#mod-8w-collapse-add-fp').click(function(){
    let html =`
    ...
    <div class="d-flex flex-row justify-content-between">
      ...
      data-target="#mod-8w-collapse-fp"
      >
    ...
    <div class="d-flex flex-column mt-1">
      <div class="mt-2"><small>Category</small></div>
        <input
          ...
          id="mod-8w-input-add-fp">
        <div class="mt-2"><small>Purpose</small></div>
        <input 
          class="font-weight-bold"
          type="text" 
          placeholder="Enter purpose" 
          id="mod-8w-input-purpose-fp">
        <a 
          href="#" 
    ...
    $('#mod-8w-collapse-fp').html(html)
    $('#mod-8w-add-fp').off()
    $('#mod-8w-add-fp').click(function(){
      if ($('#mod-8w-input-add-fp').val() != "" && $('#mod-8w-input-purpose-fp').val() != "") {
        $('#mod-8w-input-add-fp').attr('placeholder', 'Enter category')
        $('#mod-8w-input-purpose-fp').attr('placeholder', 'Enter purpose')
        fpInfo[count] = {
          "NAME": $('#mod-8w-input-add-fp').val(),
          "PURPOSE" : $('#mod-8w-input-purpose-fp').val(),
          "USED": true,
          "DEFAULT": false
        }
        count ++
        inflateFPInfoBtns()
        $('#mod-8w-input-purpose-fp, #mod-8w-input-add-fp').val("")
        scrollHorizontalEnd('mod-8w-fp')
        updatePolicy()
      } else {
        $('#mod-8w-input-add-fp').attr('placeholder', 'This field is required.')
        $('#mod-8w-input-purpose-fp').attr('placeholder', 'This field is required.')
      }
    })
    $('#mod-8w-collapse-fp').collapse('show')
  })

  inflateTPInfoBtns()
  $('#mod-8w-collapse-tpInfo-add').click(function(){
    let html =`
    <div class="card card-body">
    <div class="d-flex flex-row justify-content-between">
      <div>
      ...
      class="btn close pl-2 ml-auto"
      data-toggle="collapse" 
      data-target="#mod-8w-collapse-tpInfo"
      >
      <div class="close-char">
          <span aria-hidden="true">&times;</span>
      </div>
    ...
    <div class="d-flex flex-column mt-1">
      <div class="mt-2"><small>Category</small></div>
        <input
          ...
          type="text" 
          placeholder="Enter category" 
          id="mod-8w-input-add-tpInfo">
        <a 
          href="#" 
          class="badge badge-dark badge-pill align-self-start mt-3"
          id="mod-8w-add-tpInfo">
          Add
        </a>
      </div>
    </div>    
    ...
    $('#mod-8w-collapse-tpInfo').html(html)
    $('#mod-8w-add-tpInfo').off()
    $('#mod-8w-add-tpInfo').click(function(){
      if ($('#mod-8w-input-add-tpInfo').val() != "") {
        $('#mod-8w-input-add-tpInfo').attr('placeholder', 'Enter category' )
        tpInfo[count] = {
          "NAME": $('#mod-8w-input-add-tpInfo').val(),
          "SDKS": {},
          "DEFAULT": false
        }
        count ++
        inflateTPInfoBtns()
        $('#mod-8w-input-add-tpInfo').val("")
        scrollHorizontalEnd('mod-8w-tpInfo')
      } else {
        $('#mod-8w-input-add-tpInfo').attr('placeholder', 'This field is required.' )
      }
    })
    $('#mod-8w-collapse-tpInfo').collapse('show')
  })
}
...
function inflateCollapse(key){
  ...
  <div class="card card-body">
    <div class="d-flex flex-row">
      ...
      class="btn close pl-2 ml-auto"
      data-toggle="collapse"
      id="mod-8w-collapse-close-tpInfo"
      data-target="#mod-8w-collapse-tpInfo"
      >
      <div class="close-char">
          <span aria-hidden="true">&times;</span>
    ...
    <div class="d-flex flex-column mt-1">
      ...
      <div class="d-flex flex-column" id="mod-8w-sdks-0-info"></div>
    </div>    
  </div>
  `
  $('#mod-8w-collapse-tpInfo').html(html)
  smoothScroll()
  $('#mod-8w-collapse-close-tpInfo').off().click(function(){
    $('.tpInfo').removeClass('focus')
  })
  $('#mod-8w-delete-tpInfo').off().click(function(){
    delete tpInfo[key]
    inflateTPInfoBtns()
    $('.tpInfo').removeClass('focus')
    $('#mod-8w-collapse-tpInfo').collapse('hide')
    updatePolicy()
  })
  injectSDKS(key)
}
...
function injectSDKS(category) {
  ...
  $.each(sdks, function(key, value) {
    ...
    && tpInfo[category]["SDKS"][key]["USED"]) {
      html += `
        <button
          type="button" 
          class="btn btn-sm btn-outline-tp text-nowrap mod8w font-weight-bold"
          id="` + key + `mod8w">
          <img
      ...
      html += `
        <button
          type="button" 
          class="btn btn-sm btn-outline-secondary-alt text-nowrap mod8w"
          id="` + key + `mod8w">
          <img
          src="img/minus-square-outline.svg"
          height="18px"
  ...
  $('#mod-8w-sdks-0').html(html + '&nbsp;&nbsp')
  $('#mod-8w-scroll').click(function(){
    document.getElementById('section-2').scrollIntoView({behavior: 'smooth'})
  })
  $('.mod8w').off()
  $('.mod8w').each(function(){
    $(this).click(function(){
      $('.mod8w').removeClass('focus')
      $(this).addClass('focus')
      ...
      $('#mod-8w-usage-0').click(function(){
        ...
        injectSDKS(category)
        $('#' + key + 'mod8w').addClass('focus')
        updatePolicy()
      })
      $('#mod-8w-input-purpose-0').off()
      $('#mod-8w-input-purpose-0').on('input', function(){
        ...
        text = 'Remove'
          ...
          if (!fpInfo[key]["DEFAULT"]){
            $('#mod-8w-edit-fp')
              ...
              $('#mod-8w-edit-fp').off().click(function(){
                delete fpInfo[key]
                $('#mod-8w-collapse-fp').collapse('hide')
                inflateFPInfoBtns()
                updatePolicy()
              })
          } else {
          ...
          $('#mod-8w-close-fpInfo').off()
          $('#mod-8w-close-fpInfo').click(function(){
            $('.fpInfo').removeClass('focus')
          })

          $('#mod-8w-input-purpose-fp').off()
          $('#mod-8w-input-purpose-fp').on('input', function(){
...
function updatePolicy(){
  ...
  ($('#mod-8w-r1-yes').prop("checked") || $('#mod-8w-r0-yes').prop("checked"))) 
  {
    ...
    let t0 = 'do not'
    let t1 = ""
    if ($('#mod-8w-r3-yes').prop('checked')) {
      t0 = 'do'
      t1 = $('#mod-8w-input-0').val()
    }
